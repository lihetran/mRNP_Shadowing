...
This script mutates all C's to T's in the reads and reference sequence to allow for better alignment of APOBEC edited reads.
'''
        ...
        with pysam.AlignmentFile(mutated_bam, "wb", template=in_bam) as out_bam:
            for read in in_bam:
                ...
                if seq is not None:
                    ...
                    new_seq, positions = replaceCharacter(rc, 'C', 'T')
...
def mutateFasta(input_fasta):
    ...
    output_fasta = input_fasta.with_name(input_fasta.stem + '_mutated_CU.fasta')
    with open(output_fasta, 'w') as out_fasta:
        for record in SeqIO.parse(input_fasta, 'fasta'):
            seq = str(record.seq)
            new_seq, positions = replaceCharacter(seq, 'C', 'T')
            ...
            if read.reference_name not in ['I', 'V', 'cerENO2']: # leave these out, they're rRNA and spike-in controls
                ...
                for read_pos, ref_pos in aligned_pairs:
                    if ref_pos is not None and read_pos is not None:
                        if read_seq[read_pos] == 'T' and ref_seq[ref_pos] == 'C':
                            edits += 1
                            numAs += 1
                        elif ref_seq[ref_pos] == 'C':
                            numAs += 1

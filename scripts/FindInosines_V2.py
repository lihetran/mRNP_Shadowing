...
def main():
        ...
        with pysam.AlignmentFile(AtoGbam, "wb", template=bamFile) as outFile:
            for read in tqdm(bamFile,desc='Finding inosines'):
                if read.is_unmapped == False and read.reference_name != 'cerENO2':
                    ...
                    # #create custom tag for bam file
                    #read.set_tag('MM', inosine_positions_AG)
    ...
    mut_TC = path + '/mut_TC.fastq'
    originalReadDict_TC = {}
    with open(mut_TC, 'w') as f_out:
        for record in fastq.readFastq():
            header = record[0]
            seq = record[1]
            strand = record[2]
            ...
            newSeq, positions = replaceCharacter(seq, 'T', 'C')

            originalReadDict_TC[header.split()[0][1:]] = seq
            f_out.write(header + '\n' + newSeq + '\n' + strand + '\n' + qual + '\n')

    #### mutate reference from T-C ########
    mutated_genome_TC = 'tmp' + '/mutated_genome_TC.fa'
    refDictTC = {}
    with open(mutated_genome_TC, 'w') as f_out:
        for record in SeqIO.parse(args.fasta, 'fasta'):
            header = record.id
            seq = str(record.seq)
            rc = reverseComplement(seq)
            newSeq, positions = replaceCharacter(rc, 'T', 'C')
            refDictTC[header] = rc
            f_out.write('>' + header + '\n' + newSeq + '\n')  

    #### Align mutated reads to mutated reference (T-C) ########
    cmd1 = 'minimap2 -ax map-ont -t 20 --junc-bed ' + args.junction_bed + ' --secondary=no --for-only --MD --sam-hit-only ' + mutated_genome_TC + ' ' + mut_TC + ' > tmp/alignments.TtoC.sam'
    print('Mapping T-C reads to T-C reference')
    subprocess.call(cmd1, shell=True)


    #### remove supplementary alignments ########
    subprocess.call('samtools view -F 2048 -bo tmp/alignments.TtoC.bam tmp/alignments.TtoC.sam', shell=True)

    

    #### Find inosines in T-C alignments ########
    TtoCbam = args.outFilePrefix+'.TtoC.bam'
    with pysam.AlignmentFile('tmp/alignments.TtoC.bam','rb') as bamFile:
        #change to wb if you want to write to bam file
        with pysam.AlignmentFile(TtoCbam, "wb", template=bamFile) as outFile:
            for read in tqdm(bamFile,desc='Finding inosines'):
                if read.is_unmapped == False and read.reference_name != 'cerENO2':
                    numReads += 1
                    read_id = read.query_name
                    seq = read.query_sequence
                    original_seq = originalReadDict_TC[read_id]
                    original_ref = refDictTC[read.reference_name]
                    q_st = read.query_alignment_start
                    q_en = read.query_alignment_end
                    ref_start = read.reference_start
                    ref_end = read.reference_end
                    cigar = read.cigarstring
                    inosine_positions_TC = FindInosines(original_seq, original_ref, False, cigar, ref_start, ref_end, q_st, q_en)
                    

                    # #create custom tag for bam file
                    #read.set_tag('MM', inosine_positions_TC)
                    read.query_sequence = original_seq
                    e.append(inosine_positions_TC)

...
Updated: added global_edit_freq, global_edit_freq_capped, and n_a_positions
         columns to support fast PCA filtering and CDF plotting.

         global_edit_freq         — edit freq over all A positions in the read
                                    (no position cap applied)
         global_edit_freq_capped  — edit freq over A positions up to max_abs_idx
                                    (matches _edit_freq() in rare_read_pca_pipeline.py
                                    exactly; use this for threshold selection)
         n_a_positions            — number of non-indel A positions (uncapped)

Usage:
  python shadowingBamToParquet.py <bam_file> <reference_fasta> <output_dir> [--max_abs_idx 695]
'''
...
def read_generator(bam_path, ref_sequence, chrom, max_abs_idx=695):
    ...
    with pysam.AlignmentFile(bam_path, "rb") as bam:
        for read in bam.fetch(chrom):
            ...
            # global_edit_freq_capped (matches _edit_freq in rare_read_pca_pipeline.py)
            # Edit freq over non-indel A positions with ref pos <= max_abs_idx.
            a_idx_capped = [
                i for i, (abs_idx, c) in enumerate(zip(absolute_indices, ref_string))
                if c == 'A'
                and i < len(edit_string)
                and edit_string[i] != '2'
                and abs_idx is not None
                and abs_idx == abs_idx   # NaN check
                and int(abs_idx) <= max_abs_idx
            ]
            n_a_capped = len(a_idx_capped)
            global_edit_freq_capped = (
                sum(1 for i in a_idx_capped if edit_string[i] == '1') / n_a_capped
                if n_a_capped > 0 else 0.0
            )
            ...
            yield {
                ...
                'global_edit_freq_capped':   global_edit_freq_capped,
...
def main():
    ...
    parser.add_argument("--max_abs_idx", type=int, default=695,
                        help="Upper bound on genomic position for capped edit freq "
                             "(default: 695). Should match max_abs_idx used in the "
                             "PCA pipeline.")
    ...
    print(f"max_abs_idx for capped edit freq: {args.max_abs_idx}")
    ...
    for chrom, ref_seq in ref_dict.items():
        ...
        print(f"Finished {chrom}: {total} reads, {chunk_index + 1} chunk(s)")

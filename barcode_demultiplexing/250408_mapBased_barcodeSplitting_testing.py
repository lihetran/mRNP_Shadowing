...
Liam Tran,  April 8, 2024
This script is a modified version of the original barcode splitting script
by Marcus Viscardi. We redesigned some barcodes that are now 4nt instead of >10nt. This has resulted in some difficulties in barcode splitting. 
I'm finding that an edit distance of zero is not corresponding to exact matches. Going to troubleshoot here and potentially switch from levenshtein to hamming distance.
...
# def extract_aligned_bases(aligned_pairs, start, end, query_seq) -> str:
#     """
#     April 10, 2025 LT
#     Extract the query positions and bases from a list of aligned pairs that fall within a given range.
#     This differs from extract_seq_with_ref_positions_and_aligned_pairs in that it does not retain insertions in the query sequence.
#     This will ensure that the extracted sequence is the same length as the barcode in the reference sequence. We can now use hamming distance to compare the two sequences.
#     """
#     result = []
...
#                 break  # exit loop if position is beyond the end of the range
#     region_query_seq_list = [query_seq[i] if i else "." for i in result]
#     # region_query_seq_list = [query_seq[i] for i in result]
#     # ic(region_query_seq_list)
#     region_query_sequence = "".join(region_query_seq_list)
#     print(f"Extracted sequence: {region_query_sequence}", f"Length: {len(region_query_sequence)}")
#     return region_query_sequence
...
def extract_aligned_bases(aligned_pairs, start, end, query_seq) -> str:
    ...
    """
    result = []
    # ic(query_seq, aligned_pairs, start, end)
    for query_pos, ref_pos in aligned_pairs:
        if ref_pos is not None and ref_pos < end:
            if start <= ref_pos <= end:
                result.append(query_pos)
            elif ref_pos > end:
                break
...
def pick_exact_barcode(seq: str, barcodes: dict) -> Tuple[str, int]:
    """
    Return a matching barcode and a distance of 0 if there's an exact match.
    If no exact match is found, return 'NotDistinct' and distance -1 (or any indicator of failure).
    """
    
    seq = seq.upper()
    print(f"Input sequence: {seq}")
    print(f"Barcodes: {barcodes}")
    for name, barcode_seq in barcodes.items():
        assert len(seq) == len(barcode_seq), "Input sequence length does not match barcode length."
        if seq == barcode_seq:
            print(f"Exact match found for {name}: {seq}")
            return name, 0
...
def pick_best_barcode(input_seq: str,
    ...
    for name, bar in barcodes_dict.items():
        ...
        # dist_dict[name] = stringdist.hamming(seq_upper, bar.upper())
    ...
    with (pysam.AlignmentFile(args.bam_file, "rb") as in_bam):
        ...
        with ExitStack() as stack:
            ...
            for i, read in bam_iterator:
                ...
                if args.exact:
                    # ic(pairs, read_seq, args.ref_start, args.ref_end, read.query_name)
                    extracted_seq = extract_aligned_bases(pairs, args.ref_start, args.ref_end, read_seq)
                    barcode, distance = pick_exact_barcode(extracted_seq, barcodes)
                else:
...
def process_bam2(args: argparse.Namespace, barcodes: dict, output_bam_paths: dict, output_dir: Path):
    ...
    with (pysam.AlignmentFile(args.bam_file, "rb") as in_bam):
        ...
        for i, read in bam_iterator:
            ...
            if args.exact:
                extracted_seq = extract_aligned_bases(pairs, args.ref_start, args.ref_end, read.query_sequence)
                barcode, distance = pick_exact_barcode(extracted_seq, barcodes)
            else:
                extracted_seq = extract_seq_with_ref_positions_and_aligned_pairs(pairs,
                                                                             ...
                                                                             read.query_sequence)
                barcode, distance = pick_best_barcode(extracted_seq, barcodes, args.dist)
            # barcode, distance = pick_best_barcode2(extracted_seq, barcodes, args.dist)
    ...
    with (pysam.AlignmentFile(args.bam_file, "rb") as in_bam):
        ...
        for contig in contigs:
            ...
            for i, read in bam_iterator:
                ...
                if args.exact:
                    extracted_seq = extract_aligned_bases(aligned_pairs,
                                                         contig_start,
                                                         contig_end,
                                                         read.query_sequence)
                    barcode, distance = pick_exact_barcode(extracted_seq, barcodes)
...
def arg_parse():
    ...
    group.add_argument("-d", "--dist", type=int, 
                        help="The maximum levenshtein distance a barcode can be from the second closest barcode "
                             "to be considered distinct.")
    
    group.add_argument("--exact", action="store_true",
                        help="Use exact matching for barcodes instead of Levenshtein distance. Reads without an exact match will be assigned to the NotDistinct barcode.")

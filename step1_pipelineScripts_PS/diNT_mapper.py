'''
Script for mapping Tad8A.20 reads in 2 nucleotide space. Mutates reads and reference from A-G, C-T and maps to mutated reference.
This script then finds A-G or C-T mismatches in the alignments and calculates the efficiency of the editing enzyme.

Usage: python3 diNT_mapper.py -r <reads.fastq> -g <reference.fasta> -o <outFilePrefix>
'''
...
def FindInosines(read_seq, ref_seq, strand: bool, cigar, ref_start, ref_end, q_st, q_en):
    ...
    if strand == True: #reads are reverse complemented and mutated from A-G
        for i in range(len(top_line)):
            ...
            elif top_line[i] == 'T' and bottom_line[i] == 'C':
                numSites += 1
                inosines.append(i)

            elif bottom_line[i] == 'A' or bottom_line[i] == 'C':
                numSites += 1
    # elif strand == False: #reads are not reverse complemented and mutated from C-T
    #     for i in range(len(top_line)):
    #         if top_line[i] == 'C' and bottom_line[i] == 'T' or :
    #             numSites += 1
    #             inosines.append(i)
...
def main():
    parser = argparse.ArgumentParser(description='Find A-I, C-U edits in nanopore reads',
                                     add_help=True,
                                     prefix_chars='-')
    ...
    #### mutate and reverse complement cDNA reads from A-G and C-T ########

    originalReadDict = {}
    with open(mut, 'w') as f_out:
        for record in fastq.readFastq():
            ...
            compressed_seq, p = replaceCharacter(newSeq, 'C', 'T')
            # rc_comp = reverseComplement(compressed_seq)
            originalReadDict[header.split()[0][1:]] = rc
            f_out.write(header + '\n' + compressed_seq + '\n' + strand + '\n' + qual + '\n')

    #### mutate reference from A-G ########
    mutated_genome = 'tmp' + '/mutated_genome.fa'
    refDict = {}
    with open(mutated_genome, 'w') as f_out:
    ...
    cmd1 = 'minimap2 -ax map-ont -t 20 -k 28' + ' --secondary=no --for-only --cs="long" --sam-hit-only ' + mutated_genome + ' ' + mut + ' > tmp/alignments.compressed.sam' 
    print('Mapping compressed reads to compressed reference')
    subprocess.call(cmd1, shell=True)

    #### remove supplementary alignments ########
    subprocess.call('samtools view -F 2048 -bo tmp/alignments.compressed.bam tmp/alignments.compressed.sam', shell=True)
    ...
    mutbam = args.outFilePrefix+'.compressed.bam'
    numReads = 0
    e = []
    with pysam.AlignmentFile('tmp/alignments.compressed.bam','rb') as bamFile:
    # with pysam.AlignmentFile('tmp/alignments.AtoG.sam','rb') as samFile:
        #change to wb if you want to write to bam file
        with pysam.AlignmentFile(mutbam, "wb", template=bamFile) as outFile:
            for read in tqdm(bamFile,desc='Finding edits'):
                # if read.is_unmapped == False and read.reference_name != 'cerENO2':
                if read.is_unmapped == False:
                    numReads += 1
                    read_id = read.query_name
                    seq = read.query_sequence
                    original_seq = originalReadDict[read_id]
                    original_ref = refDict[read.reference_name]
                    ...
                    # cs = read.get_tag('cs')
                    edit_positions = FindInosines(original_seq, original_ref, True, cigar, ref_start, ref_end, q_st, q_en)
                    # read.set_tag('cs', cs)
                    read.query_sequence = original_seq
                    e.append(edit_positions)
    ...
    subprocess.call('samtools sort ' + mutbam + ' -o ' + mutbam[:-3] + 'sorted' + '.bam', shell=True)
    subprocess.call('samtools index ' + mutbam[:-3] + 'sorted.bam', shell=True)

    eff = [x for x in e if x is not None]
    print('Edit Efficiency: ' + str(sum(eff)/len(eff)))
